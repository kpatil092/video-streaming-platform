...

{
  ...
  // import { useLoaderData, Await, useNavigate, defer } from "react-router-dom";
  ...
  //   const channelsPromise = getData("/users/subscriptions/channel");
  //   const videosPromise = getData("/users/subscriptions/videos?page=1&limit=6");
  //   return defer({
  //     channelsData: channelsPromise,
  //     videosData: videosPromise,
  //   });
  // }
  // const Subscriptions = () => {
  //   const navigate = useNavigate();
  //   const observerElem = useRef(null);
  //   const queryClient = useQueryClient();
  //   const { channelsData, videosData } = useLoaderData();
  ...
  //     (entries) => {
  //       const [entry] = entries;
  //       if (entry.isIntersecting && videosData.hasNextPage) {
  //         videosData.fetchNextPage();
  //       }
  //     },
  //     [videosData]
  //   );
  ...
  //         <Await resolve={videosData}>
  //           {({ data }) => (
  //             <>
  //               <div className="flex justify-center">
  //                 <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 3xl:grid-cols-4 gap-4 mx-auto">
  //                   {data.docs.map((video, index) => (
  //                     <VideoCard
  //                       key={video._id || index}
  //                       id={video._id || 1}
  //                       thumbnail={
  ...
  //                         video.avatar || "https://via.placeholder.com/48"
  //                       }
  //                       title={video.title || "Default Video Title"}
  //                       channelName={video.channelName || "Channel Name"}
  //                       views={video.views || "0 views"}
  //                       uploadTime={format(video.createdAt) || "N/A"}
  //                       videoLength={
  //                         formatTime(parseInt(video.duration)) || "N/A"
  //                       }
  //                       small={false}
  //                     />
  //                   ))}
  ...
  //                 className="h-10 flex justify-center items-center"
  //               >
  //                 {videosData.isFetchingNextPage ? <span>Loading...</span> : null}
  //                 {!videosData.hasNextPage && !videosData.isFetchingNextPage ? (
  //                   <span>No more videos to load.</span>
  //                 ) : null}
  //               </div>
  //             </>
